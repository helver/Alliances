package Collectors::Collector_Alcatel1648;
...
sub new {
  ...
  $this->{password} = "BLU_Y0NDER";
  $this->{port} = 2001;           # TL1 port on the element
  $this->{prompt} = '/[;| ]/';    # Prompt regular expression
  $this->{timeout} = 5;           # Time to wait before a timeout
  $this->{sleeptime} = 1;         # Time to sleep between commands
  ...
  $this->{speed_specific}->{"OC12"} = {
  	...
  	"second_regex" => qr/:([A-Z]{2,4})(L),(\d+),[^,]*,NEND,([T|R])/,
  ...
  $this->{speed_specific}->{"OC3"} = {
  	...
  	"first_regex" => qr/1-DAY/,
  	"second_regex" => qr/:([A-Z]{2,3})(L),(\d+),[^,]*,[^,]*,([T|R])/,
  	"command" => "RTRV-PM-OC3",
  	"transcol" => 2,
  };
  ...
  $this->{speed_specific}->{"T3"} = {
  	"tr_code" => { "transmit" => "T", "receive" => "R" },
  	"first_regex" => qr/1-DAY/,
  	"second_regex" => qr/:([A-Z]{2,3})(L|P),(\d+),[^,]*,NEND,([T|R])/,
  	"command" => "RTRV-PM-T3",
  	"transcol" => 4,
  };
...
sub __pull_data
{
  ...
  $cmd = $speed->{command} . ":" . $this->{req}->{tid} . ":" . $this->{req}->{aid} . ":GEO::,0-UP,,,1-DAY,:PC;";
  my ($before, $lines) = $this->{connection}->print($cmd, 'COMPLD', $this->{sleeptime}, 2);
...
sub __parse_lines
{
  ...
  foreach my $line (split /\s*\n\s*/, $lines) {
    ...
    $this->__drop_message("Found 1 Day line: $line\n", 1, 0);

    # example of output:
    #    "OC3STS1-48-1,STS1:UASP,0,PRTL,NEND,RCV,1-DAY,,,N"
    #    "OC3STS1-48-1,STS1:CVP,85,PRTL,FEND,RCV,1-DAY,,,N"
    ...
    print "Made it past second regex.\n";
...
#    my ($pm, $mode2, $value, $mode4) = ($1, $2, $3, $4);
    ...
    my $params = $this->{"PARAMETERS"}->{$this->{type}->{speed}};
    
    $this->__drop_message("Transid: $transid -- " . $specs->{tr_code}->{transmit} . "--" . $specs->{tr_code}->{receive} . "\n", $this->{debug} >= 4, 0);

    my $transsy = "RECEIVE";
    if($specs->{"transcol"} == 4) {
      $transsy = ($mode4 eq $specs->{tr_code}->{transmit} ? "TRANSMIT" : "RECEIVE");
    }
    
    my $key = uc("${transsy}_${pm}${mode2}");
    print "key: $key\n";
    ...
    $this->{updates}->{$params->{uc("${transsy}_${pm}${mode2}")}} = $value;
