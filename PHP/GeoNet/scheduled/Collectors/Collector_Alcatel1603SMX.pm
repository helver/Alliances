package Collectors::Collector_Alcatel1603SMX;
...
sub new {
  ...
  $this->{speed_specific}->{"OC48"} = {
  	...
  	"first_regex" => qr/15-MIN/,
  	"second_regex" => qr/:([A-Z]{2,3})([S|L]),(\d+),[^,]*,(NEND),/,
  ...
  $this->{speed_specific}->{"STS1"} = {
  	"tr_code" => { "transmit" => "P", "receive" => "" },
  	"first_regex" => qr/15-MIN/,
  	"second_regex" => qr/:([A-Z]{2,3})(P),(\d+),[^,]*,NEND,(.)/,
...
sub __pull_data
{
  ...
  if($this->{debug} >= 4) {
    $this->__drop_message("speed: " . $this->{type}->{speed} . "\n", 1, 0);
  ...
  $cmd = $speed->{command} . ":" . $this->{req}->{tid} . ":" . $this->{req}->{aid} . ":GEO::,0-up,,,15-MIN,;";
...
#    my ($pm, $mode2, $value, $mode4) = ($1, $2, $3, $4);
    ...
    $this->__drop_message("pm: $pm, mode2: $mode2, value: $value, mode: $mode4\n", $this->{debug} >= 4, 0);
  }
