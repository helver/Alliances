package Collectors::Collector_Cisco15454;
...
sub new {
  ...
  $this->{password} = "P1KE5#PEAK";
  $this->{port} = 3082;           # TL1 port on the element
  ...
  $this->{speed_specific}->{"OC12"} = {
  	...
  	"second_regex" => qr/:([A-Z]{2,4})([S|L]),(\d+),[^,]*,NEND,([T|R|B])/,
  ...
  $this->{speed_specific}->{"OC3"} = {
  	...
  	"first_regex" => qr/1-DAY/,
  	"second_regex" => qr/:([A-Z]{2,3})(S),(\d+),[^,]*,[^,]*,([T|R|B])/,
  	"command" => "RTRV-PM-OC3",
  	"transcol" => 2,
  };
  ...
  $this->{speed_specific}->{"STS1"} = {
  	"tr_code" => { "transmit" => "R", "receive" => "T" },
  	"first_regex" => qr/1-DAY/,
  	"second_regex" => qr/:([A-Z]{2,3})(P),(\d+),[^,]*,NEND,([T|R|B])/,
...
#  $cmd = $speed->{command} . ":" . $this->{req}->{tid} . ":" . $this->{req}->{aid} . ":GEO::,0-UP,,TRMT,1-DAY;";
#  my ($xbefore, $xlines) = $this->{connection}->print($cmd, 'COMPLD', $this->{sleeptime});
...
#    my ($pm, $mode2, $value, $mode4) = ($1, $2, $3, $4);
    ...
    $weGotSomething = 1;
  	...
  	#  $this->{updates}->{$this->{PARAMETERS}->{uc("transmit_${pm}")}} = $value;
