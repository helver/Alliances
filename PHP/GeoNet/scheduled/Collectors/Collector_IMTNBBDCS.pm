package Collectors::Collector_IMTNBBDCS;
...
sub new {
  ...
  $this->{password} = "GR8#DAYG";
  $this->{username} = "CONFIG2";
  $this->{password} = "DAY#8GRG";
  ...
  $this->{speed_specific}->{"OC12"} = {
  	...
  	"second_regex" => qr/:([A-Z]{2,4})-(L|S),((\d+)|(NA)),[^,]*,NEND/,
  ...
  $this->{speed_specific}->{"STS1"} = {
  	"tr_code" => { "transmit" => "P", "receive" => "T" },
  	"first_regex" => qr/15-MIN/,
  	"second_regex" => qr/:([A-Z]{2,3})(P),((\d+)|(NA)),[^,]*,NEND,([T|R])/,
  	"command" => "RTRV-PM-STS1",
  	"transcol" => 2,
  };
  
  $this->{speed_specific}->{"STS12C"} = {
  	"tr_code" => { "transmit" => "V", "receive" => "T" },
  	"first_regex" => qr/15-MIN/,
  	"second_regex" => qr/:([A-Z]{2,3})-(V),((\d+)|(NA)),[^,]*,NEND/,
  	"command" => "RTRV-PM-STS1",
  	"transcol" => 2,
  };
  ...
  $this->{speed_specific}->{"T3"} = {
  	...
  	"first_regex" => qr/15-MIN/,
  	"second_regex" => qr/:([A-Z]{2,3})C?-(L|P),((\d+)|(NA)),[^,]*,NEND/,
  	"command" => "RTRV-PM-T3",
  	"transcol" => 2,
  };
...
sub __login
{
  ...
  if($before !~ /Open/) {
  	sleep(5);
    $this->__drop_message("Waited 10.  Now waiting 30 for Open:\n", $this->{debug} >= 1, 0);
    ($before, $lines) = $this->{connection}->print('', "Open", 0);
  }
  $this->__drop_message("Done Waiting for Open.  Saw: $lines\n", $this->{debug} >= 1, 0);
...
sub __logout
{
  ...
  $cmd = "CANC-USER:::126;";
  $this->{connection}->print($cmd, 'COMPLD', 0);
...
sub __pull_data
{
  ...
  $cmd = $speed->{command} . "::" . $this->{req}->{aid} . ":GEO::,0-up,,,15-MIN,;";
  my ($before, $lines) = $this->{connection}->print($cmd, 'COMPLD', 5);
...
#    my ($pm, $mode2, $value, undef, undef, $mode4) = ($1, $2, $3, $4, $5, $6);
    ...
    if($value eq "NA") {
      ...
      $this->{updates}->{$this->{PARAMETERS}->{uc("${st}_${pm}")}} += $mode4;
    }
