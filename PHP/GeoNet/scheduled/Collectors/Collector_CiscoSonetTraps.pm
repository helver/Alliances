package Collectors::Collector_CiscoTraps;
...
sub __pull_data
{
  ...
  $sock->send("GetTraps|" . $req->{ipaddress} . "|" . $req->{shelf} . "|" . $req->{channel} . "|" . $req->{slot_transmitter} . "\n", 10);
...
sub request
{
  ...
  return unless $this->__determine_color_and_cause;
