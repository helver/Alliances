...
sub new {
  ...
  my $port = $config->getAttribute("RequestorPort") + ($debug >= 5 ? $debug : 0);
  
  print "Opening TCP socket to " . $config->getAttribute("RequestorHost") . " on port $port\n" if($debug >= 4);
  
  my $sock = IO::Socket::INET->new(
     Proto => "tcp",
     PeerPort => $port,
     PeerAddr => $config->getAttribute("RequestorHost"),
     Type => SOCK_STREAM,
...
sub send
{
  ...
  $this->{sock}->send($dat) or die "RequestorSockWrapper::send Error: $!\n";
}
...
sub receive_data
{
  ...
  while($res !~ /\n?END\n$/) {
    ...
    while($select->can_read(1)) {
      defined($this->{sock}->recv($dat, 1024, 0)) or die "RequestorSockWrapper::recv Error: $!\n";
      print "dat: $dat\n" if($this->{debug} >= 1);
