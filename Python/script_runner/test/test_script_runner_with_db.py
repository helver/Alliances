...
class ScriptRunnerTests(unittest.TestCase):
    ...
    def tearDown(self):
        if hasattr(self, 't_job') and self.t_job:
            for res in self.t_job.results:
                self.session.update(res)
                self.session.delete(res)
                
            self.session.update(self.t_job)
            self.session.delete(self.t_job)
            self.session.flush()
            self.session.clear()
            
        if hasattr(self, 'script_runner') and self.script_runner and self.script_runner.db:
            self.script_runner.db.close()
    ...
    def testWebInfRender(self):
        ...
        newreq = bogusRequest(path='/newjob', args={'jobhref': [self.t_job.href]})
        
        self.assertEqual(newreq.path, "/newjob")
        self.assertEqual(newreq.args["jobhref"][0], self.t_job.href)
        ...
        self.assertIn(self.t_job.href, self.script_runner.active_jobs.keys())
        self.assertEqual(self.script_runner.active_jobs[self.t_job.href]['job_obj'].id, self.t_job.id)
    ...
    def testReportStatus_3(self):
        ...
        for status in status_lines:
            ...
            self.session.save_or_update(x_res)
            self.t_job.results.append(x_res)
            self.session.save_or_update(self.t_job)
        ...
        for status in status_lines:
            self.session.update(self.script_runner.active_jobs[status]['result'])
    ...
    def testInitiate(self):
        ...
        self.session.update(self.job_rec['job_obj'])
