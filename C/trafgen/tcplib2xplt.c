...
void do_up_data_file(FILE* tf1, FILE* xf1, FILE* xf2, struct data_file *dat, int j);
...
int main(int argc, char **argv)
{
	...
	while(!feof(tf1)){
	    do_up_data_file(tf1, xf1, xf2, &(data[i]), j);
	    j++;
	}
...
void do_up_data_file(FILE* tf1, FILE* xf1, FILE* xf2, struct data_file *dat, int j)
{
    ...
    *nul = '\0';
    ...
    fprintf(xf1, "box %f %f\n", (x == 0 ? x : log10(dat->points[j].x)), log10(dat->points[j].y/dat->toty));

    if(xf2) 
	fprintf(xf2, "box %f %f\n", log10(dat->points[j].x), log10(dat->points[j].y/dat->toty));

    if(j != 0) {
	fprintf(xf1, "line %f %f %f %f\n", 
		log10(dat->points[j-1].x), 
		log10(dat->points[j-1].y/dat->toty),
		log10(dat->points[j].x), 
		log10(dat->points[j].y/dat->toty));
	...
	if(xf2)
	    fprintf(xf2, "line %f %f %f %f %s\n", 
		    ...
		    log10(dat->points[j-1].y/dat->toty),
		    log10(dat->points[j].x), 
		    log10(dat->points[j].y/dat->toty), colors[cur_color]);
    } else 
...
void scale(struct data_file data[2])
{
    ...
    lcmul = lcm(data[1].toty, data[2].toty);
...
void do_correlation(FILE *xf2, FILE *sf2, struct data_file data[2])
{
	...
	for(j = 0; j < 2; j++) {
	    ...
	    while((one_run < data[j].lines) && (data[j].points[one_run].x >= temp1) && (data[j].points[one_run].x < temp2) && (data[j].points[one_run].x <= data[j].maxx)) {
		if(j == 0)
...
double range_comp(struct data_file *one, struct data_file *two, int top, int bot)
{
	...
	if(one->points[one_run].x == i) {
	    one_count++;
	    one_sum += log(((double)one->points[one_run].y));
	}

	if(two->points[two_run].x == i) {
	    two_count++;
	    two_sum += log(((double)two->points[two_run].y));
	}
    }
	...
	if(one->points[one_run].x == i) {
	    one_sum = log((double)one->points[one_run].y * 1000000) - one_ave;
/*   	    one_sum *= one_sum;   */
	} else {
/* 	    if(one_run > 0) { */
/* 		if(one->points[one_run-1].x == one->points[one_run].x) { */
/* 		    one_sum = one->points[one_run].y - one_ave; */
/* 		} else { */
/* 		    one_sum = ((double)one->points[one_run-1].y + (((double)(i - one->points[one_run-1].x)/(double)(one->points[one_run].x - one->points[one_run-1].x)) * (one->points[one_run].y - one->points[one_run-1].y))); */
/* 		    one_sum -= one_ave; */
/* 		    one_sum *= one_sum;   */
/* 		} */
/* 	    } */
...
/*    	    two_sum *= two_sum;   */
	...
	if(two->points[two_run].x == i) {
	    two_sum = log((double)two->points[two_run].y * 1000000) - two_ave;
/*    	    two_sum *= two_sum;   */
 	} else {
/* 	    if(two_run > 0) { */
/* 		if(two->points[two_run-1].x == two->points[two_run].x) { */
/* 		    two_sum = two->points[two_run].y - two_ave; */
/* 		} else { */
/* 		    two_sum = ((double)two->points[two_run-1].y + (((double)(i - two->points[two_run-1].x)/(double)(two->points[two_run].x - two->points[two_run-1].x)) * (two->points[two_run].y - two->points[two_run-1].y))); */
/* 		    two_sum -= two_ave; */
/* 		    two_sum *= two_sum;   */
/* 		} */
/* 	    } */
